...
bool check(int target, vector<int>& a, int ind, int sum) {
    if (sum == target) return true;
    if (ind == a.size() || sum > target) return false;
    if (dp[ind][sum] != -1) return dp[ind][sum];

    // Include current element
    bool include = check(target, a, ind + 1, sum + a[ind]);
    // Exclude current element
    bool exclude = check(target, a, ind + 1, sum);
