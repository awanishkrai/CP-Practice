...
bool ascY(const pair<int, pair<int, int>> &a, const pair<int, pair<int, int>> &b) {
    return a.second.second < b.second.second;
}

bool descY(const pair<int, pair<int, int>> &a, const pair<int, pair<int, int>> &b) {
    return a.second.second > b.second.second;
}
...
void solve() {
    ...
    int k = n / 2;
    vector<pair<int, pair<int, int>>> A(pts.begin(), pts.begin() + k);
    vector<pair<int, pair<int, int>>> B(pts.begin() + k, pts.end());

    sort(A.begin(), A.end(), ascY);
    sort(B.begin(), B.end(), descY);

    for (int i = 0; i < k; ++i) {
        cout << A[i].first << " " << B[i].first << "\n";
    }
}
// Main
int32_t main() {
