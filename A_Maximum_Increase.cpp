...
// Recursive function to find LIS ending at index 'ind'
int siz(const vector<int>& v, int ind) {
   if(ind < 0) return 0; // Base case
   if(ind==0) return 1; // Single element case
   if(dp[ind]!=-1)return dp[ind]; // Return memoized result
    int ans = 1; // At least the element itself
    if(v[ind]> v[ind-1]) {
        ans = 1+siz(v,ind-1); // Include the previous element if it's smaller
    }
    return dp[ind] = ans; // Memoize the result
}
