...
int check(string k, set<char>& m) {
    vector<char> arr(k.begin(), k.end());           
    set<char> unique(arr.begin(), arr.end());       
    int s = m.size() + unique.size();               

    for (int i = 0; i < arr.size(); i++) {
        m.insert(arr[i]);                           
    }

    if ((int)m.size() == s) return 1;              
    return 0;
}
...
void solve() {
    ...
    set<char> a;
    vector<char> first(v[0].begin(), v[0].end());
